<template>
  <div class="cc-segmented">
    ...
    >
      <slot
        v-if="(item as OptionsItem).slot"
        :name="(item as OptionsItem).slot"
        :item="item"
        :index="index"
      />
      <div v-else>
        {{ isObject(item) ? (item as OptionsItem).label : item }}
      </div>
    </div>
    <div
